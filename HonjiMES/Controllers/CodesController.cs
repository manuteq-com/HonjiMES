...
namespace HonjiMES.Controllers
{
    ...
    public class CodesController : Controller
    {
        ...
        public IActionResult GetQrCode()
        {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                foreach (var item in idlist)
                {
                    ...
                    var bQrCode = BarcodeHelper.CreateQrCode(txt, 70, 70);
        ...
        [HttpGet]
        public IActionResult GetUserQrCode()
        {
            var idlist = new List<int>();
            var Querylist = Request.Query["id"];
            if (!Querylist.Any())
            ...
            using (var ms = new MemoryStream())
            {
                ...
                {
                    //由左到右，由上到下照順序填滿表格
                    var Users = _context.Users.Find(item);
                    var txt = Users.Username;
                    var bQrCode = BarcodeHelper.CreateQrCode(txt, 90, 90);
                    var QrCodeimg = iTextSharp.text.Image.GetInstance(bQrCode);
                    PdfPTable itemtable = new PdfPTable(1);
                    PdfPCell borderCell1 = new PdfPCell(QrCodeimg);
                    borderCell1.FixedHeight = 40;

...
@Component({
    ...
    templateUrl: './user-qrcode.component.html',
    styleUrls: ['./user-qrcode.component.css']
})
export class UserQrcodeComponent implements OnInit, OnChanges {
    ...
    ngOnChanges() {
        ...
        this.dataSourceDB = new CustomStore({
            ...
            load: (loadOptions) => {
                // loadOptions.sort = [{ selector: 'Username', desc: false }];
    ...
    download() {
        ...
        if (rowkeys.length === 0) {
            Swal.fire({
                ...
                title: '沒有勾選任何帳戶項目',
                html: '請勾選要產生QRCode的帳戶項目',
                icon: 'warning',
                timer: 3000
            });
        } else {
            const url = '/Api/Codes/GetUserQrCode?id=' + rowkeys.toString();
            this.http.get<any>(url, { responseType: 'blob' as 'json' }).subscribe(
                (s) => {
                    ...
                    link.download = 'UserQrCode.pdf'; //瀏覽器下載時的檔案名稱
                    link.click();
                }
            );
